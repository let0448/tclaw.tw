...
| **裁判費、執行費、規費的實收金額** | **本法原定額數 ×《臺灣高等法院民事訴訟與非訟事件及強制執行費用提高徵收額數標準》的加徵**,只查本法會低估 |
| **逐年/逐縣市公告的數字**(最低生活費、遺贈稅免稅額與扣除額、最低工資、法扶無資力標準) | 該年度主管機關公告;文章須標明適用年度,或改寫成指向公告、不寫死數字 |
| 主管機關訂定的準則、指引(如職場霸凌防治措施準則、職安署預防指引) | 主管機關官網(如 laws.mol.gov.tw),MCP 多半未收錄 |
| 兩者皆查無 | 標【資料不足,無法確認】 |
...
⚠️ **數字類要多查一層,不要只查本法。** 2026-09-15 全站數字查核的 80 項差異中,有 9 項出在
這兩個坑:一是規費只查本法原定額數(民訴 77-13 的 1,000 元、77-14 的 3,000 元、強執 28-2 的
七角),漏掉提高徵收標準的加徵(實收為 1,500、4,500、千分之八);二是引用逐年公告的數字卻沒
標年度(某篇的臺中市最低生活費 17,515 不對應任何年度,115 年為 16,431,連帶三個計算範例全錯)。
落筆前先問:這個數字有沒有另一部法規在加徵?它是不是每年會變?
