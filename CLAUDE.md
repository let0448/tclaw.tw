...
- ⚠️ 舊規格為「1500–2500 字」,實際近八篇為 3,033–6,683 字,已於 2026-08-24 依實況修正。
  以下既有文章超過 5,000 字上限,列為 **legacy 例外**,新文適用新規;要不要回頭拆分未決,不順手做:
  `workplace-bullying`(7,048)、`nominee-registration-prevention`(5,358)、
  `fraud-defense`(5,291)、`high-asset-divorce-asset-tracing`(5,224)。
  ⚠️ 2026-09-15 全站數字查核後修正法律內容,字數再增(前兩篇為既有超標,後兩篇修正前即為
  5,034／5,191,同屬既有超標)。修正正確性優先於字數,未為了壓字數刪回錯誤敘述。
...
- [ ] 圖片、內鏈、外鏈正常
- [ ] 回指內鏈已補(見下節;未補完不算發布完成)

### 回指內鏈(發布完成的定義)
內鏈只靠新文往舊文連會單向累積,新文變孤兒(2026-09 盤點出 12 篇無連入)。**新文上線後,回頭在既有文章補上指向它的內鏈,才算發布完成。**
- 目標至少 2 篇連入;找不到自然位置就據實回報,不硬塞、不另起「延伸閱讀」清單
- 外科手術式:錨文字包住既有字句,錨文字用實際爭點(不用「點此」);不改法律內容、不動 H2、不改段落結構
- 排程文(未來日期):等新文實際上線後再補回指,否則舊文會出現死連結
- 查連入數:`grep -l "](/articles/{slug}/" _posts/*.md | grep -v "{slug}" | wc -l`
